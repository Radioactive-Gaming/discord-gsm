...
shard_ids = (
    [
        int(shard_id)
        for shard_id in os.getenv("APP_SHARD_IDS").replace(";", ",").split(",")
        if shard_id
    ]
    if len(os.getenv("APP_SHARD_IDS", "")) > 0
    else None
)
...
async def on_guild_join(guild: discord.Guild):
    ...
    if public:
        async with aiohttp.ClientSession() as session:
            ...
            await webhook.send(
                f"<@{client.user.id}> joined {guild.name}({guild.id}) 🎉."
            )
            return
...
async def on_guild_remove(guild: discord.Guild):
    ...
    Logger.info(
        f"{client.user} left {guild.name}({guild.id}), associated servers were deleted."
    )
...
async def on_guild_channel_delete(channel: discord.abc.GuildChannel):
    ...
    Logger.info(
        f"Channel #{channel.name}({channel.id}) deleted, associated servers were deleted."
    )
...
async def tree_sync(guild: discord.Object = None):
    ...
    try:
        ...
        if public:
            Logger.info(
                "Sync the commands to Discord. Global commands take up to 1-hour to propagate."
            )
        elif guild is not None:
            Logger.info(
                f"Sync the commands to guild {guild.id}. Guild commands propagate instantly."
            )
    except discord.ClientException as e:
        Logger.error(
            f"The client does not have an application ID. {e} {guild.id if guild else ''}"
        )
    except discord.Forbidden as e:
        Logger.error(
            f"The client does not have the applications.commands scope in the guild. {e} {guild.id if guild else ''}"
        )
...
def query_server_modal(game: GamedigGame, locale: Locale):
    ...
    query_param: dict[str, TextInput] = {
        "host": TextInput(
            label=t("modal.text_input.address.label", locale),
            placeholder=t("command.option.address", locale),
        ),
        "port": TextInput(
    ...
    modal = (
        Modal(title=title).add_item(query_param["host"]).add_item(query_param["port"])
    )
    ...
    if game["id"] == "terraria":
        ...
        query_param["host"] = TextInput(
            label="Account ID", placeholder="API key for the account id"
        )
        query_extra["_api_key"] = TextInput(
            label="API Key", placeholder="Account ID of the server"
        )
        modal.add_item(query_extra["_api_key"])
        modal.remove_item(query_param["port"])
        query_param["port"]._value = "0"
    elif game["id"] == "satisfactory":
        query_extra["_token"] = TextInput(
            label="Application Token", placeholder="Server Application token"
        )
        modal.add_item(query_extra["_token"])
        ...
        query_extra["voice_port"] = TextInput(
            label="Voice Port", placeholder="Voice port", default="9987"
        )
        ...
        query_extra["admin_password"] = TextInput(
            label="Admin Password", placeholder="admin"
        )
        modal.add_item(query_extra["api_port"])
        modal.add_item(query_extra["admin_password"])
    elif game["id"] == "tmnf":
        query_extra["username"] = TextInput(
            label="Username", placeholder="Query Username", default="User"
        )
        query_extra["password"] = TextInput(
            label="Password", placeholder="Query Password", default="User"
        )
...
def query_server_modal_handler(
    interaction: Interaction, game: GamedigGame, is_add_server: bool
):
    """Query server modal"""
    ...
    async def modal_on_submit(interaction: Interaction):
        ...
        for key in params.keys():
            if "port" in key.lower() and not gamedig.is_port_valid(str(params[key])):
                content = t(
                    "function.query_server_modal.invalid_port", interaction.locale
                )
        ...
        game_id, address, query_port = (
            game["id"],
            str(query_param["host"]),
            int(str(query_param["port"])),
        )
        ...
        if is_add_server:
            try:
                ...
                content = t(
                    "function.query_server_modal.already_exists", interaction.locale
                )
        ...
        try:
            ...
            content = t(
                "function.query_server_modal.fail_to_query", interaction.locale
            ).format(game_id=game_id, address=address, query_port=query_port)
            await interaction.followup.send(content, ephemeral=True)
            Logger.debug(
                f"Query servers: ({game_id})[{address}:{query_port}] {type(e).__name__}: {e}"
            )
            return
        ...
        if is_add_server:
            ...
            Logger.info(
                f"Successfully added {game_id} server {address}:{query_port} to #{interaction.channel.name}({interaction.channel.id})."
            )
...
@tree.command(
    name="sponsor", description="Sponsor to DiscordGSM", guilds=whitelist_guilds
)
async def command_sponsor(interaction: Interaction):
    ...
    """
    embed = Embed(
        title=title, description=description, color=discord.Color.from_rgb(88, 101, 242)
    )
    embed.add_field(
        name="❤️ Github Sponsor", value="https://github.com/sponsors/DiscordGSM"
    )
    embed.add_field(
        name="⭐ Give us a star on Github", value="https://discordgsm.com/github"
    )
...
@tree.command(
    name="queryserver",
    description="command.queryserver.description",
...
async def command_queryserver(interaction: Interaction, game_id: str):
    ...
    if game := await find_game(interaction, game_id):
        await interaction.response.send_modal(
            query_server_modal_handler(interaction, game, False)
        )


@tree.command(
    name="addserver",
    description="command.addserver.description",
    guilds=whitelist_guilds,
)
@app_commands.guild_only()
@app_commands.describe(game_id="command.option.game_id")
    ...
    if game := await find_game(interaction, game_id):
        if public:
            ...
            if len(await database.all_servers(guild_id=interaction.guild.id)) >= limit:
                content = t(
                    "command.addserver.limit_exceeded", interaction.locale
                ).format(limit=limit)
                await interaction.response.send_message(content, ephemeral=True)
                return

        await interaction.response.send_modal(
            query_server_modal_handler(interaction, game, True)
        )


@tree.command(
    name="delserver",
    description="command.delserver.description",
    guilds=whitelist_guilds,
)
@app_commands.guild_only()
...
async def command_delserver(
    interaction: Interaction,
    address: str,
    query_port: app_commands.Range[int, 0, 65535],
):
...
@tree.command(
    name="factoryreset",
    description="command.factoryreset.description",
    guilds=whitelist_guilds,
)
@app_commands.guild_only()
    ...
    async def button_callback(interaction: Interaction):
        ...
        async def purge_channel(channel_id: int):
            ...
            try:
                ...
                Logger.error(
                    f"Channel {channel.id} channel.purge discord.Forbidden {e}"
                )
            except discord.HTTPException as e:
                # Purging the messages failed.
                Logger.error(
                    f"Channel {channel.id} channel.purge discord.HTTPException {e}"
                )
...
@tree.command(
    name="moveup", description="command.moveup.description", guilds=whitelist_guilds
)
@app_commands.guild_only()
@app_commands.describe(address="command.option.address")
@app_commands.describe(query_port="command.option.query_port")
...
@tree.command(
    name="movedown", description="command.movedown.description", guilds=whitelist_guilds
)
@app_commands.guild_only()
@app_commands.describe(address="command.option.address")
@app_commands.describe(query_port="command.option.query_port")
...
async def action_move(
    interaction: Interaction, address: str, query_port: int, direction: bool
):
...
@tree.command(
    name="changestyle",
    description="command.changestyle.description",
    guilds=whitelist_guilds,
)
@app_commands.guild_only()
@app_commands.describe(address="command.option.address")
...
async def command_changestyle(
    ...
    if server := await find_server(interaction, address, query_port):
        ...
        for style_type in Styles.types():
            ...
            options.append(
                SelectOption(
                    ...
                    emoji=style.emoji,
                    default=style.id == current_style.id,
                )
            )
        ...
        content = t("command.changestyle.content", interaction.locale).format(
            address=server.address, query_port=server.query_port
        )
        await interaction.response.send_message(content, view=view, ephemeral=True)
...
@tree.command(
    name="editstyledata",
    description="command.editstyledata.description",
    guilds=whitelist_guilds,
)
@app_commands.guild_only()
@app_commands.describe(address="command.option.address")
...
async def command_editstyledata(
    ...
    if server := await find_server(interaction, address, query_port):
        ...
        title = t("command.editstyledata.modal.title", interaction.locale).format(
            address=server.address, query_port=server.query_port
        )
...
@tree.command(
    name="switch", description="command.switch.description", guilds=whitelist_guilds
)
@app_commands.guild_only()
...
async def command_switch(
    ...
    query_port: Optional[app_commands.Range[int, 0, 65535]],
):
    ...
    if servers := await find_servers(interaction, address, query_port):
        ...
        if len(servers) <= 1:
            ...
            content = t("command.switch.success", interaction.locale).format(
                count=len(servers),
                channel_id1=interaction.channel.id,
...
@tree.command(
    name="settimezone",
    description="command.settimezone.description",
    guilds=whitelist_guilds,
)
@app_commands.guild_only()
...
async def command_settimezone(
    ...
    Logger.command(
        interaction, timezone=timezone, address=address, query_port=query_port
    )

    if timezone not in timezones:
        content = t("command.settimezone.invalid", interaction.locale).format(
            timezone=timezone
        )
        await interaction.response.send_message(content, ephemeral=True)
        return
...
@tree.command(
    name="setclock", description="command.setclock.description", guilds=whitelist_guilds
)
@app_commands.guild_only()
...
@app_commands.choices(
    clock_format=[
        app_commands.Choice(name="command.choice.12_hour_clock", value=12),
...
@tree.command(
    name="setlocale",
    description="command.setlocale.description",
    guilds=whitelist_guilds,
)
@app_commands.guild_only()
...
async def command_setlocale(
    ...
    if locale not in set(str(value) for value in Locale):
        content = t("command.setlocale.invalid", interaction.locale).format(
            locale=locale
        )
        await interaction.response.send_message(content, ephemeral=True)
        return
...
@tree.command(
    name="setalert", description="command.setalert.description", guilds=whitelist_guilds
)
@app_commands.guild_only()
@app_commands.describe(address="command.option.address")
@app_commands.describe(query_port="command.option.query_port")
async def command_setalert(
    ...
    if server := await find_server(interaction, address, query_port):
        ...
        async def button1_callback(interaction: Interaction):
            ...
            text_input_webhook_url = TextInput(
                label=label,
                default=server.style_data.get("_alert_webhook_url", ""),
                required=False,
            )
            label = t("modal.text_input.webhook_content.label", interaction.locale)
            text_input_webhook_content = TextInput(
                label=label,
                default=server.style_data.get("_alert_content", ""),
                required=False,
                max_length=4000,
            )
            ...
            modal = (
                Modal(title=title)
                .add_item(text_input_webhook_url)
                .add_item(text_input_webhook_content)
            )
            ...
            async def modal_on_submit(interaction: Interaction):
                ...
                server.style_data.update(
                    {"_alert_webhook_url": webhook_url, "_alert_content": content}
                )
                await database.update_servers_style_data([server])
        ...
        async def button2_callback(interaction: Interaction):
            try:
                ...
                await interaction.response.send_message(content, ephemeral=True)
                Logger.info(
                    f"({server.game_id})[{server.address}:{server.query_port}] Send Alert Test successfully."
                )
            except NameError:
                # The URL is empty.
                ...
                Logger.error(
                    f"({server.game_id})[{server.address}:{server.query_port}] send_alert Exception {e}"
                )
        ...
        content = t("command.setalert.content", interaction.locale).format(
            address=server.address, query_port=query_port
        )
        await interaction.response.send_message(content, view=view, ephemeral=True)
...
async def find_game(interaction: Interaction, game_id: str):
    ...
    try:
        ...
        content = t("function.find_game.not_found", interaction.locale).format(
            game_id=game_id
        )
        await interaction.response.send_message(content, ephemeral=True)
        return None
...
async def find_server(interaction: Interaction, address: str, query_port: int):
    ...
    try:
        ...
        content = t("function.find_server.not_found", interaction.locale).format(
            address=address, query_port=query_port
        )
        await interaction.response.send_message(content, ephemeral=True)
        return None
...
async def find_servers(
    interaction: Interaction, address: Optional[str], query_port: Optional[int]
):
...
async def fetch_message(server: Server):
    ...
    try:
        ...
        Logger.error(
            f"({server.game_id})[{server.address}:{server.query_port}] fetch_message discord.NotFound {e}"
        )
        ...
        Logger.error(
            f"({server.game_id})[{server.address}:{server.query_port}] fetch_message discord.Forbidden {e}"
        )
        server.message_id = None
        await database.update_servers_message_id([server])
    except discord.HTTPException as e:
        ...
        Logger.error(
            f"({server.game_id})[{server.address}:{server.query_port}] fetch_message discord.HTTPException {e}"
        )
...
async def resend_channel_messages(
    interaction: Optional[Interaction], channel_id: Optional[int] = None
):
    ...
    try:
        await channel.purge(
            check=lambda m: m.author == client.user,
            before=interaction.created_at if interaction else None,
        )
    except discord.Forbidden as e:
        # You do not have proper permissions to do the actions required.
...
async def tasks_query_servers():
    ...
    if _tasks_query_servers_thread is None:
        _tasks_query_servers_thread = threading.Thread(
            target=run_in_new_loop, args=(__tasks_query_servers,)
        )
        ...
        await asyncio.gather(
            tasks_send_alert(servers),
            tasks_edit_messages(servers),
            tasks_presence_update(tasks_query_servers.current_loop),
        )
...
async def __tasks_query_servers():
    ...
    pre_query_tasks = [
        ...
        if protocol.pre_query_required and games_servers_count.get(name, 0) > 0
    ]
    ...
    percent = (
        len(pre_query_results) > 0 and int(failed / len(pre_query_results) * 100) or 0
    )
    execution_time = datetime.now().timestamp() - start_time
    Logger.debug(
        f"Pre query servers: Total = {len(pre_query_results)}, Success = {success}, Failed = {failed} ({percent}% fail), Time used = {execution_time:.2f} seconds"
    )
    ...
    execution_time = datetime.now().timestamp() - start_time
    Logger.info(
        f"Query servers: Total = {len(queried_servers)}, Success = {success}, Failed = {failed} ({percent}% fail), Time used = {execution_time:.2f} seconds"
    )


async def query_servers(
    distinct_servers: dict[tuple[str, str, int, str], list[Server]],
):
    query_tasks = [
        asyncio.create_task(query_distinct_server(servers))
        for servers in distinct_servers.values()
    ]

    async for chunks in to_chunks(
        query_tasks, int(os.getenv("TASK_QUERY_CHUNK_SIZE", "50"))
    ):
...
async def query_distinct_server(servers: list[Server]):
    ...
    try:
        ...
        status = True
        Logger.debug(
            f"Query servers: ({server.game_id})[{server.address}:{server.query_port}] Success. Ping: {result.get('ping', -1)}ms"
        )
    except Exception as e:
        result = None
        status = False
...
async def get_distinct_servers(
    servers: list[Server],
) -> dict[tuple[str, str, int, str], list[Server]]:
    ...
    Logger.debug(
        f"Query servers: Total = {total}, Distinct = {distinct}, Duplicated = {duplicated}, DNS Failed = {failed}"
    )
...
async def pre_query(protocol: Protocol):
    ...
    try:
        ...
        Logger.debug(
            f"Pre query servers: [{protocol.name}] Fail to query. {type(e).__name__}: {e}"
        )
        return False
...
async def tasks_send_alert(servers: list[Server]):
    ...
    async def send_alert_webhook(server: Server):
        ...
        try:
            ...
            Logger.info(
                f"({server.game_id})[{server.address}:{server.query_port}] Send Alert {'Online' if server.status else 'Offline'} successfully."
            )
        except NameError:
            # The Webhook URL is empty.
            pass
    ...
    def should_send_alert(server: Server):
        if server.status:
            ...
            return (
                int(server.result["raw"].get("__fail_query_count", "0"))
                == fail_query_count
            )

    alerted_servers = []
    tasks = [
        send_alert_webhook(server) for server in servers if should_send_alert(server)
    ]
...
async def tasks_fetch_messages():
    ...
    execution_time = datetime.now().timestamp() - start_time
    Logger.info(
        f"Fetch messages: Total = {len(results)}, Success = {success}, Failed = {failed} ({success and int(failed / len(results) * 100) or 0}% fail), Time used = {execution_time:.2f} seconds"
    )
...
async def tasks_edit_messages(servers: list[Server]):
    ...
    execution_time = datetime.now().timestamp() - start_time
    Logger.info(
        f"Edit messages: Total = {len(results)}, Success = {success}, Failed = {failed} ({success and int(failed / len(results) * 100) or 0}% fail), Time used = {execution_time:.2f} seconds"
    )
...
async def tasks_presence_update(current_loop: int):
    ...
    if activity_name := env("APP_ACTIVITY_NAME"):
        ...
        if advertise_type == AdvertiseType.server_count:
            ...
            if servers := await database.all_servers():
                players, bots, maxplayers = map(
                    sum, zip(*[Style.get_player_data(server) for server in servers])
                )
...
async def heroku_query():
    ...
    try:
        ...
        Logger.error(
            f"Fail to send a GET request to {url}, {e}, your discord bot will sleeps after 30 minutes of inactivity."
        )
